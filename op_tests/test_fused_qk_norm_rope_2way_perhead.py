...
@perftest()
def run_baseline_split(q0, k0, q1, k1, w_q0, w_k0, w_q1, w_k1, cos_sin0, cos_sin1,
    ...
    aiter.fused_qk_norm_rope_2way(
        ...
        B, T0, T1, Hq, Hk, D, is_interleaved, eps, q01, k01,
    )
...
@perftest()
def run_fused_perhead(q0, k0, q1, k1, w_q0, w_k0, w_q1, w_k1, cos_sin0, cos_sin1,
                     B, T0, T1, Hq, Hk, D, is_interleaved, eps):
    ...
    return aiter.fused_qk_norm_rope_2way_fp8_perhead_quant(
        q0, k0, q1, k1, w_q0, w_k0, w_q1, w_k1, cos_sin0, cos_sin1,
        B, T0, T1, Hq, Hk, D, is_interleaved, eps, out_q01, out_k01,
    )
...
def main():
    ...
    is_interleaved = False  # Qwen Image uses is_neox_style = True => is_interleaved=False
    eps = 1e-6
    ...
    bf16_diff_ph_q = (qb_bf16.to(torch.float32) - qh_bf16.to(torch.float32)).abs().max().item()
    bf16_diff_ph_k = (kb_bf16.to(torch.float32) - kh_bf16.to(torch.float32)).abs().max().item()
    ...
    print(f"  perhead: q={max_rel_err(q_ref, q_ph):.3%}  k={max_rel_err(k_ref, k_ph):.3%}")
