...
// Atoms:  extract_a/b, compute_lds_addrs, scatter_ds_read
// Phases: phase_b_prefetch, phase_a_prefetch, phase_ab_prefetch,
//         phase_compute, kstep_compute
// AGPR:   agpr_to_vgpr
//
...
// Phase 1: 4*E_M MFMAs (4 ksteps over all E_M rows) + 2 ds_reads prefetching
// the next v_b tile. PENDING = outstanding lgkmcnt from prior phase's reads.
...
// Phase 2: 4*E_M MFMAs + a_ds_read_insts ds_reads prefetching the next v_a tile
// (4 reads when E_M=2, 2 reads when E_M=1).
// PENDING = outstanding lgkmcnt from phase_b_prefetch's reads.
