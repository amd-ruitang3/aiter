...
def compile_mxscale_gemm(
    *,
    ...
    if wave_specialized_tdm and num_warps < 4:
        raise ValueError(
            f"wave_specialized_tdm requires at least 4 waves, got {num_warps}"
        )
    if use_ab_split_scale_buffer_load and not wave_specialized_tdm:
        raise ValueError(
            "scale_load_path='buffer_lds_stage_ab_split' requires wave_specialized_tdm=True"
        )
    ...
    if use_ab_split_scale_buffer_load:
        if tile_m % 2 != 0:
            raise ValueError(
                f"buffer_lds_stage_ab_split requires even tile_m, got {tile_m}"
            )
        if tile_n % 32 != 0:
            raise ValueError(
                f"buffer_lds_stage_ab_split requires tile_n divisible by 32, got {tile_n}"
            )
    ...
    def kernel_mxscale_gemm(
        ...
        def compute_tile_fp8_quadrant(
            ...
            for ks in range_constexpr(k_wmma_steps):
                ...
                _emit_group(
                    0, _fp8_half_wn, a_top_frags, b_right_frags, a_scales, b_scales
                )
                _emit_group(
                    _fp8_half_wm,
                    _fp8_half_wn,
        ...
        def hot_loop_scheduler_fp8_quadrant():
            ...
            for _ks in range_constexpr(k_wmma_steps):
                if const_expr(_ks == 0):
                    rocdl.sched_dsrd(
                        _b_left_bundle_loads + _a_scale_loads + _a_all_loads
                    )
                else:
        ...
        if const_expr(wave_specialized_tdm):
            active_pred_const = arith.select(
                tdm_wave_id < fx.Int32(4), fx.Int32(1), fx.Int32(0)
            )
        ...
        if const_expr(use_scale_buffer_load):
            ...
            def _dma_scale_tile_to_lds(
                ...
                for batch in range_constexpr(
                    (total_bytes + block_threads * _scale_dma_bytes - 1)
                    // (block_threads * _scale_dma_bytes)
                ):
                    batch_byte = batch * block_threads * _scale_dma_bytes
                    copy_byte = arith.index(batch_byte) + tx * arith.index(
                        _scale_dma_bytes
                    )
                    if copy_byte < arith.index(total_bytes):
                        ...
                        global_byte = (
                            (global_row_base + row) * arith.index(row_stride)
                            + global_col_base
                            + col
                        )
                        global_ptr = buffer_ops.create_llvm_ptr(
                            global_base + global_byte, address_space=1
                        )
                        lds_ptr = buffer_ops.create_llvm_ptr(
                            memref_dialect.extract_aligned_pointer_as_index(lds_mem)
                            + copy_byte,
                            address_space=3,
                        )
        ...
        def _issue_ab_tdm(load_stage, addr_a, addr_b):
            dg0_a = _pack_dg0(
                pred_const, stages_a_lds_addr[load_stage], addr_a, addr_hi_a
            )
            dg0_b = _pack_dg0(
                pred_const, stages_b_lds_addr[load_stage], addr_b, addr_hi_b
            )
            issue_tdm_loads(
        ...
        if const_expr(
            ...
            and (not use_scale_buffer_load or use_ab_split_scale_buffer_load)
        ):

            def _issue_active_tdm(
                load_stage, addr_box, scale_k_box=None, k_prefetch=None
            ):
                dg0 = _pack_dg0(
                    active_pred_const,
                ...
                for loop_iter, state in range(0, loop_iters, 1, init=init_args):
                    ...
                    for buf_idx in range_constexpr(num_buffers):
                        ...
                        def _mid_tdm_split_scale_dma(
                            ...
                            _issue_active_tdm(
                                _ls, _ab, scale_k_box=_scale_k, k_prefetch=_k_off
                            )
            ...
            if const_expr(_outstanding == -1):
                ...
                if const_expr(_load_stage is not None):
                    ...
                    if const_expr(use_ab_split_scale_buffer_load):
                        ...
                        def _tail_mid_split_scale_dma(
                            _ls=_load_stage, _ab=_tail_addr_box, _scale_k=_tail_scale_k
                        ):
                        ...
                        def _tail_mid_scale_dma(
                            _ls=_load_stage, _ab=_tail_ab, _scale_k=_tail_scale_k
                        ):
                            _issue_ab_tdm(_ls, _ab[0][0], _ab[1][0])
                            _ab[0][0] = _ab[0][0] + adv_a_i32
                            _ab[1][0] = _ab[1][0] + adv_b_i32
